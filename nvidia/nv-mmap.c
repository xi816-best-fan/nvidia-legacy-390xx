...
)
{
    ...
    if (!NV_IS_CTL_DEVICE(nv))
    {
        ...
        { 
			...
			vma->vm_page_prot = vm_get_page_prot(vma->vm_flags | VM_MIXEDMAP);
    ...
    else
    {
        ...
        vma->vm_page_prot = vm_get_page_prot(vma->vm_flags | (VM_IO | VM_LOCKED | VM_RESERVED));
        vma->vm_page_prot = vm_get_page_prot(vma->vm_flags | (VM_DONTEXPAND | VM_DONTDUMP));
    }
    ...
    if ((prot & NV_PROTECT_WRITEABLE) == 0)
    {
        ...
        vma->vm_page_prot = vm_get_page_prot(vma->vm_flags & ~VM_MAYWRITE);
    }
